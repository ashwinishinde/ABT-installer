...
class abt(xmlrpc.XMLRPC): 
	...
	def xmlrpc_getAllExportedOrganisations(self):
		...
		* Output:
        ...
        def xmlrpc_copyPDF(self,queryParams):
                """
                input: PDF filename
                copy pdf from /mnt/sdcard to user's file system
                """
                print "We are about to pull file"
                if os.path.exists("/opt/abt/adb") == True:
			os.system("mkdir $HOME/ABT_PDF/")
			os.system("sudo chmod 777 $HOME/ABT_PDF/")
                        print "adb is present"
			os.system("export PATH=/opt/abt:$PATH")
                        os.system("/opt/abt/adb -e pull /mnt/sdcard/"+queryParams+".pdf $HOME/ABT_PDF/")
                else:
                        print "adb is not present"
                return "success"

	def xmlrpc_pushexport(self):
		home= os.getenv("HOME")
		print home
		if os.path.exists(home+"/export/"):
			os.system("/opt/abt/adb -e push $HOME/export/ /mnt/sdcard/export/")
			os.system("sudo cp $HOME/export/ /opt/abt/ -R")
		else:
			print"No file found" 
		return "success"

	def xmlrpc_copyCSV(self,queryParams):
                """
                input: PDF filename
                copy pdf from /mnt/sdcard to user's file system
                """
                print "We are about to pull file"
                if os.path.exists("/opt/abt/adb") == True:
			os.system("mkdir $HOME/ABT_CSV/")
			os.system("sudo chmod 777 $HOME/ABT_CSV/")
                        print "adb is present"
			os.system("export PATH=/opt/abt:$PATH")
                        os.system("/opt/abt/adb -e pull /mnt/sdcard/"+queryParams+".csv $HOME/ABT_CSV/")
                else:
                        print "adb is not present"
                return "success"

	def xmlrpc_copyDocs(self):
                """
              
		copy Doc folder from /ABTcore/ to user's file system
		"""
		print "We are about to push folder"
		if os.path.exists("/opt/abt/adb") == True:
			print "In copydoc funtion"
			os.system("export PATH=/opt/abt:$PATH")
			os.system("pwd")
			os.system("sudo cp /opt/abt/doc $HOME -R")    
			os.system("/opt/abt/adb -e push $HOME/doc/ /data/local/doc")
		else:
			print "adb is not present"
		return "success"

	def xmlrpc_copyexport(self):
                """
                input: PDF filename
                copy export folder to user's file system
                """
                print "We are about to push file"
		if os.path.exists("/opt/abt/adb") == True:

			os.system("sudo rm -rf $HOME/export")   
			os.system("sudo cp /opt/abt/export/ $HOME -R")     
			#os.system("sudo chmod 722 $HOME/export/ -R")
                        os.system("/opt/abt/adb -e push $HOME/export/ /mnt/sdcard/export/") # push export file to /mnt/sdcard/ so that later it can imported
